source("functions/INLA_Mmodel_icar.R")
source("functions/INLA_Mmodel_lcar.R")
source("functions/INLA_Mmodel_pcar.R")
...
MCAR_INLA_ST <- function(carto=NULL, data=NULL, ID.area=NULL, ID.year=NULL, ID.disease=NULL,
  ...
  form <- paste(form, "+", paste(paste0("f(idy.",1:J,", model='generic0', Cmatrix=Rt, rankdef=dif, constr=FALSE, extraconstr=list(A=A.constr.t, e=rep(0,nrow(A.constr.t))), hyper=list(prec=list(prior=sdunif)))"), collapse=" + "), sep=" ")
  if(interaction!="none"){
    form <- paste(form, "+", paste(paste0("f(idz.",1:J,", model='generic0', Cmatrix=R, rankdef=r.def, constr=FALSE, extraconstr=list(A=A.constr.st, e=rep(0,nrow(A.constr.st))), hyper=list(prec=list(prior=sdunif)))"), collapse=" + "), sep=" ")
  }
  ...
  if(temporal=="rw2" & interaction=="TypeII"){
    A.constr.t <- rbind(matrix(1,1,T),matrix(1:T,1,T))
  }else{
    A.constr.t <- matrix(1,1,T)
  }
  ...
  idy.<- paste0("idy.", 1:J)
  for(j in 1:J){
    data.INLA[data.INLA$ID.disease==j, idy.[j]] <- data.INLA[data.INLA$ID.disease==j, "ID.year"]
  }
  idz.<- paste0("idz.", 1:J)
  for(j in 1:J){
    data.INLA[data.INLA$ID.disease==j, idz.[j]] <-  (data.INLA[data.INLA$ID.disease==j, "ID.year"]-1)*S + data.INLA[data.INLA$ID.disease==j, "ID.area"]
  }
  ...
  Mmodel.compute <- Mmodel_compute_cor(Model, n.sample=10000)
...
compute.summary <- function(marginal){
  ...
  if(any(class(o[[1]])=="error")){
    aux <- data.frame(NA,NA,NA,NA,NA,NA,NA)
    colnames(aux) <- c("mean","sd","0.025quant","0.5quant","0.975quant","mode","1 cdf")
  }
...
Mmodel_compute_cor <- function(model, n.sample=10000){
  ...
  if(any(class(o[[1]])=="error")){
    ...
    rownames(summary.cor) <- paste("rho.s",apply(combn(J,2), 2, function(x) paste0(x, collapse="")),sep="")
    ...
    rownames(summary.var) <- paste("var.s",1:J,sep="")
      
    marginals.var <- as.list(rep(NA,J))
    names(marginals.var) <- rownames(summary.var)
  }
    
  Mmodel.compute <- list(summary.cor=summary.cor.s, marginals.cor=marginals.cor.s,
                         summary.var=summary.var.s, marginals.var=marginals.var.s)
